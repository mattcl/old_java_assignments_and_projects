...
/**
 * Credit for this code goes to Albert from the Robocode Wiki. It allows you to
 * precisely determine your position by simulating movement using Robocode physics.
 * 
 * This could be approximated using a much simpler method
 * 
 * @author Albert
 *
 */
public class MovSim {
    ...
    public static MovSimStat[] futurePos(int steps, AdvancedRobot b) { return futurePos(steps, b, defaultMaxVelocity, defaultMaxTurnRate); }
    ...
    public static MovSimStat[] futurePos(int steps, Point2D.Double location, double velocity, double maxVel, double heading, double distanceRemaining, double turnRemaining, double bfWidth, double bfHeight) {
        return futurePos(steps, location.x, location.y, velocity, maxVel, heading, distanceRemaining, turnRemaining, defaultMaxTurnRate, bfWidth, bfHeight);
    }
    
    public static MovSimStat[] futurePos(int steps, double x, double y, 
            double velocity, double maxVelocity, double heading, 
            double distanceRemaining, double angleToTurn, double maxTurnRate, 
        ...
        for (int i=0; i<steps; i++) {
            ...
            if (distanceRemaining != 0.0 || velocity != 0.0) { 
                ...
                if (x<WALL_TOLLERANCE || y<WALL_TOLLERANCE || x>battleFieldW-WALL_TOLLERANCE || y>battleFieldH-WALL_TOLLERANCE) {
                    distanceRemaining = 0;
                    angleToTurn = 0;
                    velocity = 0;
                    moveDirection = 0;
                    x = Math.max(WALL_TOLLERANCE,Math.min(battleFieldW-WALL_TOLLERANCE,x));
                    y = Math.max(WALL_TOLLERANCE,Math.min(battleFieldH-WALL_TOLLERANCE,y));
                }
            }
            //add position
